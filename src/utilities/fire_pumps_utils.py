...
DATA_FILE = os.path.join(os.path.dirname(__file__), "..", "data", "fire_pumps_inventory.json")

def load_fire_engine_inventory():
    """Read the portable engine inventory from disk."""
    with open(DATA_FILE, "r") as f:
        return json.load(f)


def save_fire_new_engine(form_data):
    """Append a new engine entry when the user selects 'Other'."""
    inventory = load_fire_engine_inventory()
...
def save_fire_new_model(form_data):
    """
